...
def build_executable(debug=False, onedir=True, onefile=False, build_distribution=False):
    ...
    # NOTE: Do NOT force-include libexpat/libssl/libcrypto via --include-data-files.
    # Unlike PyInstaller's --add-binary (see build_pyinstaller_linux.py), Nuitka treats
    # those as plain data files and then collides with its own dependency walker, which
    # already bundles the same .so as a DLL:
    #   "FATAL: data file to be placed in distribution as 'libexpat.so.1' conflicts with
    #    dll 'libexpat.so.1'."
    # Inside a Conda env the walker resolves these to $CONDA_PREFIX/lib automatically, so
    # explicit pinning is both redundant and the cause of the conflict.
