...
def find_project_root():
    ...
    # Project root is the parent of the build_system directory
    project_root = script_dir.parent
    if (project_root / "src").exists():
        return project_root

    # Search upwards for src directory as fallback
...
def get_build_script_path(target_platform):
    ...
    if target_platform == "windows":
        return project_root / "build_system" / "build_windows_executable.py"
    else:
        return project_root / "build_system" / "build_linux_executable.py"
    ...
    for subdir in subdirs_to_clean:
        ...
        if subdir_path.exists():
            ...
            try:
                ...
                # Small delay to let Windows release the directory
                import time
                time.sleep(0.5)
                subdir_path.mkdir()
            except PermissionError:
                print(f"\n[X] Error: No se pudo limpiar la carpeta '{subdir}'.")
                print(f"    Esto ocurre porque el programa '{subdir}/baby_project_manager.exe' está abierto.")
                print("    Por favor, CIERRA la aplicación y el explorador de archivos, luego intenta de nuevo.")
                sys.exit(1)
            except Exception as e:
                print(f"   [!] Error inesperado al limpiar {subdir}: {e}")
                sys.exit(1)
...
### Windows
```bash
python build_scripts/build_windows_executable.py --clean
```

### Linux/macOS
```bash
python build_scripts/build_linux_executable.py --clean
```
