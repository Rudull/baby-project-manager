...
- **Windows:**
  - `build_windows_executable.py` - Creates Windows executable (.exe) with PyInstaller
...
- **Linux/macOS:**
  - `build_linux_executable.py` - Creates Linux/macOS executable
...
```cmd
python build_windows_executable.py --clean
```
...
Ensure your virtual environment or Conda environment is active before building:
...
# Then run the build from build_system/
cd build_system
python build_linux_executable.py --clean
```
...
Building in Conda Linux environments can lead to library mismatches (SSL or Expat errors). The `build_linux_executable.py` script automatically attempts to detect and bundle these libraries from your $CONDA_PREFIX.

If you encounter `ImportError: undefined symbol` or `SSL module not available`:
1. Ensure your Conda environment is active: `conda activate baby`
2. Run the build script again with `--clean`.

### Shared Libraries
On some Linux distributions, you may need additional system libraries for Qt to run:
```bash
sudo apt install libxcb-cursor0 libnss3 libatk-bridge2.0-0
```
