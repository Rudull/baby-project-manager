...
def create_spec_file(debug=False, onedir=False):
    ...
    # Detect Conda environment and libraries
    conda_prefix = os.environ.get('CONDA_PREFIX')
    binaries = []
    
    if conda_prefix and platform.system().lower() == 'linux':
        conda_lib_path = Path(conda_prefix) / "lib"
        
        # Critical libs to bundle from Conda to avoid symbol mismatches or missing modules
        libs_to_bundle = ["libexpat.so.1", "libssl.so.3", "libcrypto.so.3"]
        
        for lib_name in libs_to_bundle:
...
def run_with_spinner(cmd, cwd=None):
    ...
    i = 0
    while not done:
        sys.stdout.write(f"\rBuild en progreso... {spinner[i % len(spinner)]}")
        sys.stdout.flush()
        time.sleep(0.2)
        i += 1
    sys.stdout.write("\rBuild finalizado.                      \n")
    thread.join()
