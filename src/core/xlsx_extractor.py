"""xlsx_extractor.py
Extracts task data from Excel project files (.xlsx, .xls).
"""
from __future__ import annotations

import logging
import re
import unicodedata
from datetime import datetime
from typing import Any

import pandas as pd

from utils.filter_util import is_start_end_task

logger = logging.getLogger("bpm.xlsx_extractor")


class XLSXReader:
    def __init__(self) -> None:
        self.column_mappings: dict[str, list[str]] = {
            "task_id": ["id", "task id", "taskid", "#", "código", "codigo"],
            "name": [
                "name",
                "task name",
                "taskname",
                "activity",
                "description",
                "nombre",
                "tarea",
                "actividad",
                "descripcion",
                "descripción",
            ],
            "start_date": [
                "start",
                "start date",
                "startdate",
                "begin date",
                "fecha inicio",
                "inicio",
                "fecha de inicio",
                "fecha_inicio",
                "start_date",
                "fecha de comienzo",
                "comienzo",
            ],
            "end_date": [
                "end",
                "finish",
                "end date",
                "enddate",
                "finish date",
                "fecha fin",
                "fin",
                "fecha de fin",
                "fecha final",
                "fecha_fin",
                "fecha terminación",
                "terminación",
            ],
            "level": [
                "level",
                "outline level",
                "wbs",
                "hierarchy",
                "nivel",
                "jerarquía",
                "jerarquia",
                "nivel_",
            ],
        }

    def normalize_column_name(self, name: str) -> str:
        """Elimina acentos y caracteres especiales, y convierte a minúsculas."""
        name = "".join(
            c for c in unicodedata.normalize("NFD", name)
            if unicodedata.category(c) != "Mn"
        )
        return name.lower().strip()

    def identify_columns(self, df: pd.DataFrame) -> dict[str, str]:
        """Identifica las columnas relevantes del DataFrame."""
        identified_columns: dict[str, str] = {}
        df.columns = [self.normalize_column_name(str(col)) for col in df.columns]

        for key, possible_names in self.column_mappings.items():
            for col in df.columns:
                normalized_possible = [self.normalize_column_name(n) for n in possible_names]
                if col in normalized_possible or any(n in col for n in normalized_possible):
                    identified_columns[key] = col
                    break

        # Fallback para fechas usando tipo de columna
        date_columns = [
            col for col in df.columns if pd.api.types.is_datetime64_any_dtype(df[col])
        ]
        if "start_date" not in identified_columns and date_columns:
            identified_columns["start_date"] = date_columns[0]
        if "end_date" not in identified_columns and len(date_columns) > 1:
            identified_columns["end_date"] = date_columns[1]

        return identified_columns

    def format_date(self, date_value: Any) -> str:
        """Convierte un valor de celda Excel a cadena dd/mm/yyyy."""
        if pd.isna(date_value):
            return ""
        try:
            if isinstance(date_value, (int, float)):
                return pd.Timestamp.fromordinal(
                    datetime(1900, 1, 1).toordinal() + int(date_value) - 2
                ).strftime("%d/%m/%Y")

            if isinstance(date_value, (datetime, pd.Timestamp)):
                return date_value.strftime("%d/%m/%Y")

            if isinstance(date_value, str):
                date_value = date_value.strip()
                for fmt in ("%Y-%m-%d", "%d/%m/%Y", "%m/%d/%Y", "%d-%m-%Y"):
                    try:
                        return datetime.strptime(date_value, fmt).strftime("%d/%m/%Y")
                    except ValueError:
                        continue

            return str(date_value)
        except Exception as err:
            logger.debug("Unexpected error formatting date '%s': %s", date_value, err)
            return str(date_value)

    def compare_dates(self, date1: str, date2: str) -> bool:
        """Compara dos fechas en formato string dd/mm/yyyy."""
        if not date1 or not date2:
            return False
        try:
            return (
                datetime.strptime(date1, "%d/%m/%Y")
                == datetime.strptime(date2, "%d/%m/%Y")
            )
        except ValueError as err:
            logger.debug(
                "Date comparison failed '%s' vs '%s': %s", date1, date2, err
            )
            return False

    def determine_outline_level(
        self, row: pd.Series, columns: dict[str, str], text: str
    ) -> int:
        """Determina el nivel jerárquico de una tarea."""
        if "level" in columns:
            try:
                level_value = row[columns["level"]]
                if not pd.isna(level_value):
                    if isinstance(level_value, (int, float)):
                        return int(level_value)
                    if isinstance(level_value, str):
                        level_str = re.sub(r"[^\d]", "", level_value)
                        if level_str:
                            return int(level_str)
            except (ValueError, TypeError) as err:
                logger.debug("Could not parse level column value: %s", err)

        leading_spaces = len(text) - len(text.lstrip())
        return leading_spaces // 2 if leading_spaces > 0 else 0

    def read_xlsx(self, file_path: str) -> list[dict[str, Any]]:
        """Lee un archivo Excel y extrae las tareas."""
        try:
            df_head = pd.read_excel(file_path, nrows=2, header=None)
            first_row_is_header = self._is_header_row(df_head.iloc[0])
            header_row = 0 if first_row_is_header else 1
            df = pd.read_excel(file_path, header=header_row)

            columns = self.identify_columns(df)
            if not columns:
                raise ValueError("No se pudieron identificar las columnas necesarias")

            tasks: list[dict[str, Any]] = []
            for idx, row in df.iterrows():
                try:
                    if "name" not in columns:
                        continue

                    task_name = str(row[columns["name"]]).strip()
                    if pd.isna(task_name) or not task_name:
                        continue

                    start_date = ""
                    end_date = ""
                    if "start_date" in columns:
                        start_date = self.format_date(row[columns["start_date"]])
                    if "end_date" in columns:
                        end_date = self.format_date(row[columns["end_date"]])

                    if is_start_end_task(task_name) or self.compare_dates(
                        start_date, end_date
                    ):
                        continue

                    outline_level = self.determine_outline_level(row, columns, task_name)

                    task_id = ""
                    if "task_id" in columns and not pd.isna(row[columns["task_id"]]):
                        task_id = str(row[columns["task_id"]])

                    task: dict[str, Any] = {
                        "task_id": task_id,
                        "name": task_name,
                        "start_date": start_date,
                        "end_date": end_date,
                        "level": str(outline_level),
                        "outline_level": outline_level,
                        "indentation": outline_level,
                    }
                    tasks.append(task)

                except Exception as err:
                    logger.debug("Skipping row %s due to error: %s", idx, err)
                    continue

            return tasks

        except Exception as err:
            logger.error(
                "Failed to read xlsx file '%s': %s", file_path, err, exc_info=True
            )
            return []

    def _is_header_row(self, row: pd.Series) -> bool:
        """Verifica si una fila parece un encabezado de columna."""
        non_numeric_count = 0
        for cell in row:
            cell_str = str(cell).strip()
            if any(
                keyword in cell_str.lower()
                for keyword in ["id", "nombre", "name", "fecha", "start", "end", "nivel", "level"]
            ):
                return True
            if not cell_str.isdigit() and len(cell_str) > 3:
                non_numeric_count += 1
        return non_numeric_count >= len(row) / 2
